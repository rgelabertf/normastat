...
export default function App() {
  ...
  const handleProcessPasted = () => {
    setErrorMsg(null);
    if (!pasteText.trim()) {
      setErrorMsg("La casilla de texto est├í vac├¡a. Por favor pega tus datos primero.");
      return;
    }
    const numbers = parseNumbersFromString(pasteText);
    if (numbers.length < 5) {
      setErrorMsg("Se requieren al menos 5 datos num├®ricos v├ílidos para procesar las pruebas estad├¡sticas.");
      return;
    }
    setRawData(numbers);
    setCsvColumns(null);
  ...
  const exportResultsToCSV = () => {
    ...
    csvRows.push(["Tama├▒o de Muestra", stats.n.toString(), "N (Total de observaciones validas)"]);
  ...
  const handleFileParse = (text: string, fileName: string) => {
    ...
    if (validKeys.length === 0) {
      setErrorMsg("No se encontraron columnas v├ílidas con al menos 5 datos num├®ricos en el archivo CSV.");
      return;
    }
    ...
    <div className="bg-zinc-50 text-zinc-900 dark:bg-[#0F1115] dark:text-[#E2E8F0] min-h-screen flex flex-col transition-colors duration-200">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              title="Descargar esta herramienta como un archivo HTML ├║nico que funciona completamente offline"
            >
          ...
          <div className="space-y-8 animate-fade-in">
              ...
              <div className="text-right">
                {level === "pri" ? (
                  <span className="text-[11px] font-semibold text-emerald-600 dark:text-green-400 bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-200/50 dark:border-emerald-900/35 px-3 py-1.5 rounded-full">
                    Ô£ô Enfoque Minimalista: Toma de decisiones r├ípida y directa
                  </span>
                ) : (
                  <span className="text-[11px] font-semibold text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-950/20 border border-blue-200/50 dark:border-blue-900/35 px-3 py-1.5 rounded-full">
                    Ô£ô Modo Avanzado: Explicaciones te├│ricas completas, manuales y chat IA
                  </span>
                )}
              </div>
            </div>
            ...
            <div className="bg-white dark:bg-[#161B22] border border-zinc-200 dark:border-[#30363D] p-6 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-sm">
              <div className="space-y-1">
                <h2 className="text-lg font-bold text-zinc-900 dark:text-white">Toma de Decisiones Cient├¡ficas</h2>
                <p className="text-xs text-zinc-500 dark:text-gray-400">
                  Las pruebas eval├║an si tu muestra difiere significativamente de una campana te├│rica de Gauss. Un valor de <strong className="text-emerald-600 dark:text-green-400">p &gt; 0.05</strong> respalda la normalidad.
                </p>
              </div>
              <div className="flex items-center gap-2 text-xs bg-zinc-50 dark:bg-[#0D1117] px-3.5 py-2 border border-zinc-200 dark:border-[#30363D] rounded-lg">
                ...
                <span className="font-semibold text-zinc-600 dark:text-gray-300">C├ílculos anal├¡ticos del lado cliente</span>
              </div>
            </div>
            ...
            <div className="grid lg:grid-cols-12 gap-8 items-start">
              ...
              <div className="lg:col-span-5 space-y-6">
                ...
                <div className="bg-white dark:bg-[#161B22] border border-zinc-200 dark:border-[#30363D] rounded-xl p-5 shadow-sm space-y-4">
                  ...
                  >
                    ...
                    <p className="text-xs font-semibold text-zinc-700 dark:text-gray-300">
                      Arrastra tu archivo CSV/TXT aqu├¡ o haz clic
                    </p>
                ...
                <div className="bg-white dark:bg-[#161B22] border border-zinc-200 dark:border-[#30363D] rounded-xl p-5 shadow-sm space-y-4">
                  ...
                  <textarea
                    ...
                    placeholder="Pega tu columna de datos num├®ricos. Ejemplo:&#10;173.2&#10;168.5&#10;174.1&#10;169.3..."
              ...
              <div className="lg:col-span-7 space-y-6">
                    ...
                    <div>
                      <div className="flex items-center justify-between gap-1">
                        <div className="flex items-center gap-1.5">
                          ...
                          {level === "pro" && (
                            <Tooltip 
                              title="Potencia Estad├¡stica (Shapiro-Francia)" 
                              ...
                              details={[
                                "Especialmente dise├▒ada para muestras de tama├▒o medio y grande.",
                                "Basada en la correlaci├│n lineal (W') entre los cuantiles ordenados de la muestra y los cuantiles te├│ricos.",
                                "W' pr├│ximo a 1 indica un alto ajuste a la normalidad te├│rica."
                              ]}
                              align="left"
                            />
                          )}
                    ...
                    <div>
                      <div className="flex items-center justify-between gap-1">
                        <div className="flex items-center gap-1.5">
                          ...
                          {level === "pro" && (
                            <Tooltip 
                              title="Distancia Emp├¡rica (Kolmogorov-Smirnov)" 
                              description="La prueba KS eval├║a la normalidad comparando la distribuci├│n acumulada observada frente a la te├│rica mediante una m├®trica de distancia espacial."
                              details={[
                                "El estad├¡stico D representa la distancia m├íxima absoluta vertical entre ambas funciones de distribuci├│n.",
                                "Un valor D peque├▒o respalda la hip├│tesis nula de normalidad.",
                                "Sensible a cambios en el centro, la distribuci├│n y la forma de la distribuci├│n."
                              ]}
                              align="center"
                            />
                          )}
                    ...
                    <div>
                      <div className="flex items-center justify-between gap-1">
                        <div className="flex items-center gap-1.5">
                          ...
                          {level === "pro" && (
                            <Tooltip 
                              title="Momentos de Distribuci├│n (Jarque-Bera)" 
                              description="La prueba JB se basa en los momentos estandarizados de tercer y cuarto orden para determinar si los datos se comportan de manera sim├®trica y mesoc├║rtica."
                              details={[
                                "Eval├║a de manera conjunta la asimetr├¡a (sesgo = 0) y el exceso de curtosis (curtosis = 0).",
                                "Estad├¡stico JB = (n/6) * [S┬▓ + (K-3)┬▓ / 4].",
                                "Especialmente efectiva para muestras grandes; sensible a valores at├¡picos extremos."
                              ]}
                              align="right"
                            />
                          )}
                ...
                {/* Criterios Fundamentales para la Toma de Decisi├│n (Principiante / Profesional) */}
                <div className="bg-white dark:bg-[#161B22] border border-zinc-200 dark:border-[#30363D] rounded-xl p-5 shadow-sm space-y-4">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <h3 className="text-xs font-bold uppercase tracking-wider text-zinc-900 dark:text-zinc-50">
                          Criterios Fundamentales de Decisi├│n
                        </h3>
                        <p className="text-[10px] text-zinc-400 dark:text-gray-500 font-medium">Gu├¡a simplificada para sustentar tu decisi├│n cient├¡fica</p>
                      </div>
                    </div>
                    <div>
                      {(() => {
                        ...
                        if (passedCount >= 2) {
                          return (
                            <span className="px-3 py-1 text-xs font-bold rounded-full bg-emerald-50 dark:bg-emerald-950/20 text-emerald-600 dark:text-green-400 border border-emerald-200/40 dark:border-emerald-900/30">
                              Ô£ô SUPUESTO CUMPLIDO (Normalidad Aceptada)
                            </span>
                          );
                        } else {
                          return (
                            <span className="px-3 py-1 text-xs font-bold rounded-full bg-amber-50 dark:bg-amber-950/20 text-amber-600 dark:text-amber-400 border border-amber-200/40 dark:border-amber-900/30">
                              Ô£ù DESVIACI├ôN DETECTADA (No Normal)
                            </span>
                          );
                        }
                      })()}
                  ...
                  <div className="grid md:grid-cols-2 gap-6 text-xs leading-relaxed">
                    <div className="space-y-3">
                      <h4 className="font-bold text-zinc-700 dark:text-gray-300 flex items-center gap-1.5">
                        ...
                        1. Regla de Oro del p-valor (Significaci├│n)
                      </h4>
                      <p className="text-zinc-500 dark:text-gray-400 text-[11px]">
                        Para aceptar la normalidad estad├¡stica, buscamos un p-valor mayor a <strong className="text-zinc-800 dark:text-white">0.05</strong>. Si es menor, la muestra se desv├¡a del comportamiento de Gauss.
                      </p>
                      <div className="bg-zinc-50 dark:bg-[#0D1117] p-3 rounded-lg border border-zinc-100 dark:border-[#2D333D] space-y-1.5 font-mono text-[10px]">
                        <div className="flex justify-between items-center">
                          ...
                          <span className={swResult.isNormal ? "text-emerald-600 dark:text-green-400 font-bold" : "text-amber-600 dark:text-amber-400 font-bold"}>
                            p = {swResult.pValue < 0.0001 ? "< 0.0001" : swResult.pValue.toFixed(4)} ({swResult.isNormal ? "Ô£ô Normal" : "Ô£ù No Normal"})
                          </span>
                        </div>
                        <div className="flex justify-between items-center">
                          ...
                          <span className={ksResult.isNormal ? "text-emerald-600 dark:text-green-400 font-bold" : "text-amber-600 dark:text-amber-400 font-bold"}>
                            p = {ksResult.pValue < 0.0001 ? "< 0.0001" : ksResult.pValue.toFixed(4)} ({ksResult.isNormal ? "Ô£ô Normal" : "Ô£ù No Normal"})
                          </span>
                        </div>
                        <div className="flex justify-between items-center">
                          ...
                          <span className={jbResult.isNormal ? "text-emerald-600 dark:text-green-400 font-bold" : "text-amber-600 dark:text-amber-400 font-bold"}>
                            p = {jbResult.pValue < 0.0001 ? "< 0.0001" : jbResult.pValue.toFixed(4)} ({jbResult.isNormal ? "Ô£ô Normal" : "Ô£ù No Normal"})
                          </span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="space-y-3">
                      <h4 className="font-bold text-zinc-700 dark:text-gray-300 flex items-center gap-1.5">
                        <span className="w-1.5 h-1.5 rounded-full bg-blue-500"></span>
                        2. ┬┐Qu├® camino estad├¡stico seguir ahora?
                      </h4>
                      <p className="text-zinc-500 dark:text-gray-400 text-[11px]">
                        Dependiendo del veredicto final sobre la normalidad de tu muestra de datos, debes seleccionar las pruebas estad├¡sticas correctas:
                      </p>
                        ...
                        if (passedCount >= 2) {
                          return (
                            <div className="bg-emerald-50/40 dark:bg-emerald-950/10 border border-emerald-100 dark:border-emerald-900/30 p-3 rounded-lg text-[11px] text-emerald-800 dark:text-emerald-350 space-y-1">
                              <span className="font-bold block">Ô£ô Recomendaci├│n: Pruebas Param├®tricas</span>
                              <span>Tus datos son aptos para aplicar pruebas de alto rendimiento como: <strong>T de Student, ANOVA de un factor, Coeficiente de Pearson</strong> o regresiones lineales cl├ísicas.</span>
                            </div>
                          );
                        } else {
                          return (
                            <div className="bg-amber-50/40 dark:bg-amber-950/10 border border-amber-100 dark:border-amber-900/30 p-3 rounded-lg text-[11px] text-amber-800 dark:text-amber-350 space-y-1">
                              <span className="font-bold block">ÔÜá Recomendaci├│n: Pruebas No Param├®tricas</span>
                              <span>Se recomienda aplicar alternativas que no asumen normalidad como: <strong>U de Mann-Whitney, Wilcoxon, Kruskal-Wallis o Correlaci├│n de Spearman</strong>. Tambi├®n puedes aplicar transformaciones matem├íticas (e.g. logaritmo natural).</span>
                            </div>
                          );
                        }
                      })()}
                ...
                <div className="bg-white dark:bg-[#161B22] border border-zinc-200 dark:border-[#30363D] rounded-xl p-5 shadow-sm space-y-4">
                  <div className="flex items-center justify-between flex-wrap gap-2">
                    <h3 className="text-xs font-bold uppercase tracking-wider text-zinc-400 dark:text-gray-500 flex items-center gap-2">
                      ...
                      Estad├¡sticos Descriptivos de la Muestra
                    </h3>
                    {level === "pro" ? (
                      <button
                        id="btn-export-csv"
                      ...
                      <span className="text-[10px] font-mono text-zinc-400 dark:text-gray-500">M├®tricas auxiliares</span>
                    )}
                  </div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    ...
                    <div className="bg-zinc-50 dark:bg-[#0D1117] p-3 rounded-lg border border-zinc-100 dark:border-[#30363D] text-center">
                      <span className="block text-[9px] uppercase font-bold text-zinc-400 dark:text-gray-550 tracking-wider">Media (╬╝)</span>
                      <span className="block text-sm font-black font-mono mt-1 text-zinc-800 dark:text-white">{stats.mean.toFixed(2)}</span>
                    </div>

                    <div className="bg-zinc-50 dark:bg-[#0D1117] p-3 rounded-lg border border-zinc-100 dark:border-[#30363D] text-center">
                      <span className="block text-[9px] uppercase font-bold text-zinc-400 dark:text-gray-550 tracking-wider">Desv. Est├índar (¤â)</span>
                    ...
                    <div className="bg-zinc-50 dark:bg-[#0D1117] p-3 rounded-lg border border-zinc-100 dark:border-[#30363D] text-center">
                      <span className="block text-[9px] uppercase font-bold text-zinc-400 dark:text-gray-550 tracking-wider">M├¡nimo</span>
                      <span className="block text-sm font-black font-mono mt-1 text-zinc-800 dark:text-white">{stats.min.toFixed(2)}</span>
                    </div>

                    <div className="bg-zinc-50 dark:bg-[#0D1117] p-3 rounded-lg border border-zinc-100 dark:border-[#30363D] text-center">
                      <span className="block text-[9px] uppercase font-bold text-zinc-400 dark:text-gray-550 tracking-wider">M├íximo</span>
                      <span className="block text-sm font-black font-mono mt-1 text-zinc-800 dark:text-white">{stats.max.toFixed(2)}</span>
                    </div>

                    <div className="bg-zinc-50 dark:bg-[#0D1117] p-3 rounded-lg border border-zinc-100 dark:border-[#30363D] text-center">
                      <span className="block text-[9px] uppercase font-bold text-zinc-400 dark:text-gray-550 tracking-wider">Asimetr├¡a (Sesgo)</span>
      ...
      <footer className="border-t border-zinc-200 dark:border-[#2D333D] bg-white dark:bg-[#161B22]/40 mt-16 py-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center text-xs text-zinc-400 dark:text-gray-550 space-y-2">
          <p>┬® 2026 Rolando Gelabert Fern├índez ÔÇö Universidad Aut├│noma del Carmen</p>
          <p>Desarrollada por Rolando Gelabert ┬À Licencia MIT</p>
          <p className="mt-1 font-mono text-[10px]">Todos los algoritmos de Shapiro-Francia, Kolmogorov-Smirnov y Jarque-Bera operan bajo f├│rmulas matem├íticas nativas de alto rendimiento.</p>
        </div>
      </footer>
